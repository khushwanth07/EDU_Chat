...
def generate_profound_answer(question, email_context, pdf_context, session_id):
    """
    Generate an answer using OpenAI API with conversational memory.
    """
    # Retrieve conversation history (last 5 exchanges)
    history = conversation_history.get(session_id, [])[-5:]
...
def handle_query():
    ...
    if not email_context and not pdf_context:
        return jsonify({"response": "No relevant information found.", "session_id": session_id})

    # Generate response using OpenAI with conversation memory
    response_text = generate_profound_answer(question, email_context, pdf_context, session_id)
